...
class WPSL_Thumb {
	...
	public function column_content( $column_slug, $post_id ) {
		if ( '_post_thumbnail' == $column_slug ) {
			...
			$fill = has_post_thumbnail( $post_id ) ? '<img width="43" height="43" class="attachment-thumbnail wp-post-image" src="' . get_the_post_thumbnail_url( $post_id, 'thumbnail' ) . '" alt="">' : '<i class="dashicons dashicons-plus"></i>';
			?>
